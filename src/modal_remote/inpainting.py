...
class InpaintingModel:
    ...
    def load_model(self):
        ...
        self.pipe = pipe
        print(
            "✅ Model (AutoPipelineForInpainting) and LoRA weights loaded successfully."
        )
    ...
    def _preprocess_image(
        ...
        if strategy == PREPROCESS_REPLACE_NOISE:
            noise = (
                np.random.randint(0, 256, image_np.shape, dtype=np.uint8) - 128
            ) * noise_scale + 128
            image_np[mask_bool] = noise[mask_bool]
            preprocessed_image = Image.fromarray(image_np)
        elif strategy == PREPROCESS_ADD_NOISE:
            noise = (
                np.random.randint(-127, 128, image_np.shape, dtype=np.int16)
                * noise_scale
            )
            np.stack([mask_bool] * 3, axis=-1) if image_np.ndim == 3 else mask_bool
            # Add noise only to masked area, clip to valid range
            masked_pixels = image_np[mask_bool]
            noisy_pixels = np.clip(
                masked_pixels.astype(np.int16) + noise[mask_bool], 0, 255
            ).astype(np.uint8)
    ...
    def _postprocess_grayscale_inpaint(
        self,
    ...
    @modal.method()
    def inference(
        self, image: Image.Image, mask_image: Image.Image, config: InpaintingConfig
    ) -> Image.Image:
        """
        ...
        if config.grayscale and original_image_for_post is not None:
            ...
            original_resized = original_image_for_post.resize(
                output_size, Image.Resampling.LANCZOS
            )
            mask_resized = mask_image.resize(output_size, Image.Resampling.NEAREST)
            result = self._postprocess_grayscale_inpaint(
                result, original_resized, mask_resized
            )
